...
def store_new_file_content(py_filename_, base_name_, stone_color_, x_coordinate_, y_coordinate_, z_coordinate_):
    csv_path = os.path.join(py_filename_, base_name_ + "new_content_{take}.csv")
    ...
    for item in v:
        note_lines.append([str(value) for value in item])
...
def main_loop():
    ...
    while True:
        try:
            ...
            py_filename, _ = os.path.splitext(os.path.basename(__file__))
